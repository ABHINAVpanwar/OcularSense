...
def train_model():
    ...
    for epoch in range(epochs):
        epoch_percent = (epoch + 1) / epochs * 100
        print(f"Epoch {epoch + 1}/{epochs} ({epoch_percent:.2f}% completed)")
        print("-" * 30)
        ...
        for batch_idx, (images, labels) in enumerate(train_loader):
            batch_percent = (batch_idx + 1) / len(train_loader) * 100
            print(f"Epoch {epoch + 1} - {batch_percent:.2f}% completed", end='\r')
